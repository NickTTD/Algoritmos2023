...
class Lista():
    ...
    def searchObjeto(self, search_value, criterio=None): ##Igual que search pero devuelve el objeto en ves de la posición
        found_object = None
        first = 0
        last = self.size() - 1
        while (first <= last and found_object is None):
            middle = (first + last) // 2
            if search_value == criterio_comparacion(self.__elements[middle], criterio):
                found_object = self.__elements[middle]
            elif search_value > criterio_comparacion(self.__elements[middle], criterio):
                first = middle + 1
            else:
                last = middle - 1
